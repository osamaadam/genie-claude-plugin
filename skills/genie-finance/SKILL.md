...
- Use `get_net_worth` for a current net-worth snapshot. Report its primary currency, `total`, `valued_at`, included accounts, excluded accounts, and `unvalued_items` as returned.
...
- `get_net_worth` is complete only when `is_complete` is true. If it is false, explain that only included accounts contribute to the reported total; excluded or unvalued accounts remain separately listed and are not silently treated as zero. Always disclose `valued_at` and any account or item valuation/FX dates supplied by the response.
- For `list_investment_holdings`, identify the provider and account for each position, preserve the flat holding list, disclose `is_complete=false`, and report native quote valuation dates and any account-level FX dates when present.
